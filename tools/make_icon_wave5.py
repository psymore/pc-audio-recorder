...
YELLOW = (250, 204, 21)   #                  #facc15  (middle block)
RED = (229, 72, 77)       # COLOR_RECORDING  #e5484d  (last block)
SS = 1024
HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "..", "icons", "Wave 5 blocks.ico")
BARS = 5
...
# Pixel-grid layouts, all coordinates integer; widths and heights even so everything is centred exactly.
NATIVE = {
    16: layout(16, 2, 1, [6, 10, 14, 10, 6], 1),
    24: layout(24, 4, 1, [10, 16, 22, 16, 10], 0),
    32: layout(32, 4, 2, [12, 20, 30, 20, 12], 2),
    48: layout(48, 8, 2, [16, 28, 42, 28, 16], 0),
}
...
def draw_smooth(size):
    ...
    d = ImageDraw.Draw(im)
    heights, w, gap = [320, 560, 860, 560, 320], 150, 50
    x = (SS - (BARS * w + (BARS - 1) * gap)) // 2
    for h, color in zip(heights, BAR_COLORS):
        d.rounded_rectangle((x, (SS - h) // 2, x + w, (SS + h) // 2), radius=36, fill=color)
        x += w + gap
...
sheet.save(os.path.join(HERE, "preview_wave5.png"))
